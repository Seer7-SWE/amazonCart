...
const Register = () => {
  ...
  const handleInsertProfile = async (user) => {
    const { name, phone, state, city } = formData;
    ...
    if (fetchError && fetchError.code !== 'PGRST116') {
      ...
      alert("Error checking user profile.");
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (error) {
      ...
      // User created, but may need to confirm email
      if (!data.user.confirmed_at) {
        alert("Check your email to confirm your account first.");
      } else {
